...
const InventoryItems = ({ product }) => {
    const { _id, name, price, quantity, img, supplierName, description } = product;
    return (
        <div className="col-md-4 g-4">
            <div className='shadow p-3 text-center'>
                ...
                <p>{description}</p>
                <button>
                    <Link className='text-decoration-none text-xl rounded bg-sky-700 text-white fw-bold px-3 py-1 ' to={`/update/${_id}`}>Stock Update</Link>
                </button>
            </div>
        </div>
    );
